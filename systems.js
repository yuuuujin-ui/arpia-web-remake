...
window.ARPIA_SYS=(()=>{
 ...
 function alliesFor(s,encounter,spec){if(spec?.allies)return spec.allies.map(a=>({...a,atb:30,st:{}}));
  ...
  if(encounter==='scoll14')list.push({id:'naomi',name:'나오미',sprite:A+'animation/npc_008_도트_나오미/idle.gif',hp:380,maxHp:380,atk:30,atb:30,skill:'attack',st:{}});
  ...
  if(['snake12','waitmonsters'].includes(encounter))list.push({id:'kesno',name:'케스노',sprite:A+'animation/npc_003_도트_케스노/idle.gif',hp:360,maxHp:360,atk:26,atb:30,skill:'attack',st:{}});
 ...
 function enemyTurn(c,e){const s=c.s,b=c.b;b.enemyTurns++;const t=beginTurn(e,e.name);if(e.hp<=0){b.log=t.log.trim();c.finish();return true;}if(t.skip){b.log=t.log.trim();b.lock=.5;c.render();return false;}
  if(e.breathAttack&&!e.breathReady){e.breathTurns=(e.breathTurns||0)+1;if(e.breathTurns%3===0){e.breathReady=true;b.log='어스 드래곤의 목주머니가 크게 부풀어 오릅니다! 다음 불꽃을 대기로 방어하세요.';b.lock=.6;c.render();return false;}}
  const breath=!!e.breathReady,protectedByGuard=!!b.guard;e.breathReady=false;
  const magic=breath||D.MAGIC_ATTACKERS.has(e.name)||!!e.magic;if(magic&&e.st?.silence){b.log=`${e.name}은(는) 침묵 상태라 마법을 쓰지 못한다.`;b.lock=.5;c.render();return false;}
  const focus=b.boss&&e.sprite==='kesno'&&b.turn%3===0?8:0;const raw=Math.max(1,Math.round((e.atk+focus)*atkMul(e)*(breath?2.3:1)*(b.spec?.mustLose?99:1)));
  ...
  else{const stats=c.stats();let d=b.guard?Math.ceil(raw*.38):Math.max(1,raw-Math.floor((magic?stats.magicDefense:stats.physicalDefense)*.18));d=damageTo({st:b.p,hp:s.hp},d);if(b.spec?.mustLose)d=Math.max(d,s.hp);if(b.p?.reflect&&!b.guard){const r=Math.round(d*.5);e.hp=Math.max(0,e.hp-r);log+=` 오토 반사 ${r}.`;}s.hp=Math.max(0,s.hp-d);if(breath&&!protectedByGuard&&s.hp>0){applyStatus({st:b.p},'burn',3);log+=' 불꽃이 몸에 달라붙었습니다.';}b.guard=false;log+=` ${e.name}의 ${magic?'마법':'공격'}! ${d} 피해.`;
 ...
 function renderBattleUI(c){const b=c.b,s=c.s,$=c.$;const list=actionList(c);
  ...
  b.enemies.forEach((e,i)=>{const bt=document.createElement('button');bt.className='enemy-card'+(i===c.sel()?' selected':'');bt.disabled=e.hp<=0;bt.innerHTML=`<strong>${e.name}</strong><small>${ELS[e.element]||'비속성'} · ${e.hp}/${e.maxHp} ${statusText(e)}</small><div class="meter"><i style="width:${e.hp/e.maxHp*100}%"></i></div>`;bt.onclick=()=>{c.select(i);renderBattleUI(c);};$('enemy-cards').appendChild(bt);});
 ...
 function bind(kind,a,arg){const s=a.state,$=id=>document.getElementById(id);const q=sel=>document.querySelectorAll(sel);const re=(k=kind,g=arg)=>a.openPanel(k,g);
  ...
  q('[data-col]').forEach(b=>b.onclick=()=>{const o=D.COLOSSEUM.find(x=>x.id===b.dataset.col);X.encounters['col_'+o.id]={name:'콜로세움 · '+o.name,intro:`딕의 진행으로 ${o.name}와 대전합니다.`,bg:'assets/maps-hires/colosseum.png',colosseum:o.id,freeBattle:true,xp:Math.round(o.enemies.reduce((t,e)=>t+e.hp,0)/4),gold:o.gold,sp:0,enemies:o.enemies.map((e,j)=>({...e,maxHp:e.hp,atb:j*10}))};a.close();a.battle('col_'+o.id);});
