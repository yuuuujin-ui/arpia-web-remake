...
window.ARPIA_FREE=(()=>{
 ...
 const defs=[
  ...
  {id:'flame_hunt',title:'불꽃병사 퇴치',giver:'flameTeacher',scene:'classroom',minStage:62,kind:'hunt',target:'flamePatrol',targetScene:'firevillage',max:10,summary:'거인의 오두막 근처를 배회하는 불꽃병사를 10마리 물리친다.',reward:'선행 점수 50',sp:50},
  ...
  {id:'pet_contest1',title:'펫 대회 1차전',giver:'contestMaster',scene:'petcenter',minStage:86,minLevel:10,kind:'boss',target:'petGate1',targetScene:'petcenter',summary:'펫과 함께 첫 번째 다섯 조련사의 연속 대결에 도전한다.',reward:'선행 점수 100 · 펫 성장식',sp:100},
 ...
 Object.assign(x.npcs,{
  ...
  laura:{name:'라우라 촌장',anim:'npc_078_도트_아다'},joker:{name:'조커',portrait:'other/조커.png'},flameTeacher:{name:'불꽃마법 선생님',anim:'npc_009_도트_아론',portrait:'teacher/아론.png'},
  puki:{name:'푸키',anim:'npc_103_도트_푸키'},hubert:{name:'휴버트',anim:'npc_023_도트_발디'},ugauga:{name:'다친 우가우가',anim:'npc_026_도트_도비엘'},
  ryoma:{name:'료마',anim:'npc_009_도트_아론'},bandMole:{name:'두더지 악단의 단원',anim:'npc_024_도트_코비'},iceChief:{name:'얼음마을 촌장',anim:'npc_115_도트_케리',portraitPath:'assets/portraits/humphrey.webp'},
  contestMaster:{name:'펫 대회 진행자',anim:'npc_038_도트_멜리'},earthSpiritKing:{name:'대지정령왕',artPath:'assets/earth-spirit-king.webp',height:105},
  iceSpiritKing:{name:'얼음정령왕',artPath:'assets/ice-spirit-king.webp',height:105},flamePatrol:{name:'불꽃병사 무리',artPath:'assets/flame-soldier.png',height:78},ghostHorse:{name:'데런 미로의 귀신'},petGate1:{name:'펫 대회 1차 접수'},petGate2:{name:'펫 대회 2차 접수'},archmageSeal:{name:'대마도사 시험의 문'}
 });
 ...
 function progress(s,d){const v=val(s,d),c=count(s,d);if(d.id==='mail')return mailSteps[Math.min(v,4)];if(d.id==='egg')return eggSteps[Math.min(v,9)]+(v===2?` · ${c}/30`:``);if(v===9)return d.repeatable?`완료 ${s['fd_'+d.id]||1}회 · 다시 의뢰 가능`:'완료';if(v===2)return `${d.giver==='morris'?'교장실':'의뢰인'}에게 완료 보고`;if(!v)return available(s,d)?'의뢰를 받을 수 있음':`잠김 · ${d.minLevel?`Lv.${d.minLevel} · `:''}제${d.minStage>=169?'11':d.minStage>=126?'8':d.minStage>=86?'6':'4'}화 이후`;if(d.kind==='hunt')return `${d.summary} · ${c}/${d.max}`;if(d.kind==='collect')return `${d.summary} · `+Object.entries(d.need).map(([id,n])=>`${window.ARPIA_DATA.ITEMS[id].name} ${window.ARPIA_SYS?window.ARPIA_SYS.inv.count(s,id):0}/${n}`).join(', ');if(d.kind==='visit')return `${x.npcs[d.target]?.name} 찾기 · ${d.targetScene}`;return `${x.npcs[d.target]?.name}에게 도전 · ${d.targetScene}`;}
 ...
 x.decorate=(sc,s,n,p)=>{decorate(sc,s,n,p);migrate(s);
  if(s.stage>=62){if(sc.id==='lobby')add(sc,n,'aron',530,320);if(sc.id==='classroom')add(sc,n,'flameTeacher',710,215);}
  if(s.stage>=86&&sc.id==='infirmary')add(sc,n,'buma',255,204);
  if(s.stage>=126){if(sc.id==='judah')add(sc,n,'puki',365,150);if(sc.id==='petcenter'){add(sc,n,'hubert',480,430);add(sc,n,'contestMaster',260,365);}if(sc.id==='classroom')add(sc,n,'ryoma',646,198);if(sc.id==='kingdom')add(sc,n,'matilda',546,590);}
 ...
 function chainTarget(s,e){const m=val(s,defs[0]),g=val(s,defs[1]);return(m===1&&e.id==='conrad')||(m===2&&e.id==='mailDiamond')||(m===3&&e.id==='conrad')||(g===1&&e.id==='abdullah')||(g===2&&e.id==='abdullah')||(g===3&&e.id==='garuda')||(g===4&&e.id==='shiva')||(g===5&&e.id==='eggStone')||(g===6&&e.id==='garuda')||(g===7&&e.id==='abdullah')||(g===8&&e.id==='buma');}
 function objective(s,e){migrate(s);if(chainTarget(s,e))return true;return defs.some(d=>{const v=val(s,d);return(available(s,d)&&e.id===d.giver)||(v===2&&e.id===d.giver)||(v===1&&['visit','boss','hunt'].includes(d.kind)&&e.id===d.target);});}
 ...
 function handleMail(e,a){const s=a.state,v=val(s,defs[0]);if(available(s,defs[0])&&e.id==='hina'){a.missionOffer(defs[0].title,'hina','우체국에서 없어진 우편물을 찾고 있대. 콘라드 아저씨를 도와드릴래?',()=>accept(defs[0],a));return true;}let next=0,msg='';if(v===1&&e.id==='conrad'){next=2;msg='배달하던 다이아몬드 원석을 북쪽에서 잃어버렸단다. 독수리 마을 근처를 찾아봐 주렴.';}if(v===2&&e.id==='mailDiamond'){next=3;msg='풀 사이에서 우편물 꼬리표가 붙은 다이아몬드 원석을 찾았다.';}if(v===3&&e.id==='conrad'){finish(defs[0],a);return true;}if(!next)return false;a.talk([[e.id==='mailDiamond'?'narrator':e.id,msg]],()=>{s.fm_mail=next;s.freeMail=next;a.save();a.refresh();});return true;}
 function handleEgg(e,a){const s=a.state,v=val(s,defs[1]);if(available(s,defs[1])&&e.id==='buma'){a.missionOffer(defs[1].title,'buma','건강센터에 쓸 알이 떨어졌어요. 압둘라에게 공급을 부탁해 주시겠어요?',()=>accept(defs[1],a));return true;}if(!chainTarget(s,e)||v===0)return false;if(v===2&&count(s,defs[1])<30){a.talk([['abdullah',`아직 ${count(s,defs[1])}/30마리요. 필드의 몬스터를 더 물리치고 오시오.`]]);return true;}if(v===8&&e.id==='buma'){s.potions+=3;finish(defs[1],a);return true;}const msgs={1:'일을 도와줄 사람이 필요했소. 주변 필드 몬스터 30마리를 물리쳐 주시오.',2:'수고했소. 알은 독수리 마을의 가루다에게 부탁하면 되오.',3:'알을 내어주고 싶지만 마나석이 부족하구나. 시바에게 물어보렴.',4:'학교 지하의 납품 상자에 마나석이 남아 있을 거야.',5:'지하 상자에서 건강센터 납품용 마나석을 챙겼다.',6:'고맙구나. 이 알을 압둘라에게 전해 주렴.',7:'알을 건강센터로 안전하게 보내겠소. 부마에게 알려 주시오.'};a.talk([[e.id==='eggStone'?'narrator':e.id,msgs[v]]],()=>{s.fm_egg=v+1;s.freeEgg=v+1;a.save();a.refresh();});return true;}
 function interact(e,a){const s=a.state;migrate(s);if(handleMail(e,a)||handleEgg(e,a))return true;for(const d of defs.slice(2)){const v=val(s,d);if(available(s,d)&&e.id===d.giver){a.missionOffer(d.title,d.giver,d.summary+` 보상은 ${d.reward}${d.repeatable?'이며 완료 후 다시 도전할 수 있어.':'이야.'}`,()=>accept(d,a));return true;}if(v===2&&e.id===d.giver){finish(d,a);return true;}if(v===1&&d.kind==='visit'&&e.id===d.target){a.talk([[d.target,`찾아와 줘서 고마워. 이제 ${x.npcs[d.giver].name}에게 무사하다고 전해 줘.`]],()=>{s['fm_'+d.id]=2;a.save();a.refresh();});return true;}if(v===1&&d.kind==='hunt'&&d.target===e.id){a.talk([[d.giver,`현재 ${count(s,d)}/${d.max}마리를 물리쳤어. 남은 무리도 조심하렴.`],['you','이번 무리도 맡겨 주세요!']],()=>a.battle('free_patrol_'+d.id));return true;}if(v===1&&d.kind==='boss'&&e.id===d.target){a.talk([[d.giver,'마력의 흐름을 읽고 펫과 함께 침착하게 맞서렴.'],['you','준비됐어요. 시작할게요!']],()=>a.battle('free_'+d.id));return true;}}
  return false;
 }
 function victory(s,b){migrate(s);const killed=b.enemies?.length||1;if(val(s,defs[1])===2){s.fc_egg=Math.min(30,count(s,defs[1])+killed);s.freeEggKills=s.fc_egg;}for(const d of defs)if(d.kind==='hunt'&&val(s,d)===1){s['fc_'+d.id]=Math.min(d.max,count(s,d)+killed);if(s['fc_'+d.id]>=d.max)s['fm_'+d.id]=2;}const id=b.spec?.freeMission;if(id){const d=defs.find(x=>x.id===id);if(d&&val(s,d)===1)s['fm_'+id]=2;}}
 const battle=(id,name,sprite,hp,atk,bg='assets/maps-hires/forest-battle.webp',enemies)=>x.encounters['free_'+id]={name,intro:`프리 미션 · ${name}`,bg,freeMission:id,freeBattle:true,xp:Math.round(hp/3),gold:45,sp:0,enemies:enemies||[{name,element:id==='ice_king'?1:id==='earth_king'?2:0,hp,maxHp:hp,atk,atb:5,sprite}]};
 ...
 x.encounters.free_patrol_flame_hunt={name:'불꽃병사 순찰대',intro:'거인의 오두막 근처를 지키는 불꽃병사들이 달려듭니다.',bg:'assets/maps-hires/forest-battle.webp',freeBattle:true,xp:85,gold:35,sp:0,enemies:[{name:'불꽃병사',element:0,hp:170,maxHp:170,atk:15,atb:0,sprite:'flameSoldier'},{name:'불꽃병사',element:0,hp:170,maxHp:170,atk:15,atb:10,sprite:'flameSoldier'},{name:'불꽃병사 대장',element:0,hp:230,maxHp:230,atk:18,atb:18,sprite:'flameSoldier'}]};
 x.questItems=s=>[...items(s),...(val(s,defs[0])===3?[['잃어버린 우편물','콘라드에게 돌려줄 다이아몬드 원석']]:[]),...(val(s,defs[1])===6?[['납품용 마나석','가루다에게 전할 재료']]:[]),...(val(s,defs[1])===7?[['건강센터의 알','압둘라에게 배송을 부탁하자']]:[]),...(val(s,defs[4])===1?[['조커의 약 재료','누에열매·랄라열매·루비 원석·수중초'],['마법 휴지','글루글루 신전에서 찾은 반짝이는 휴지']]:[])];
