document.getElementById('test-run').onclick=async()=>{
 ...
 try{
  const G=ARPIA,x=ARPIA_EXTRA;for(let i=0;!G.ready&&i<100;i++)await new Promise(r=>setTimeout(r,100));G.newGame();const s=G.state,c=ARPIA_MIDTERM.chapters.find(c=>c.number===32);
  ...
  ok(s.papaDayComplete32,'papa day');ok(!s.inv.mt_akaniaFlower32&&!s.inv.mt_starDust32&&!s.inv.mt_cake32,'gift items');out.textContent=logs.join('\n')+'\nALL PASS: episode 32, telescope, stardust, two ambushes and grandfather.';
