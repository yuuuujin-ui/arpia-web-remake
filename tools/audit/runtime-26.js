document.getElementById('test-run').onclick=async()=>{
 ...
 try{
  const G=ARPIA,x=ARPIA_EXTRA;for(let i=0;!G.ready&&i<100;i++)await new Promise(r=>setTimeout(r,100));G.newGame();const s=G.state,c=ARPIA_MIDTERM.chapters.find(c=>c.number===26);
  ...
  for(let tries=0;s.stage<c.end&&tries<70;tries++){const r=c.steps[s.stage-c.start],at=s.stage;G.close();G.travel(r.scene);const targets=G.scene.entities.filter(e=>e.id===r.npc);ok(targets.length===1,'target '+r.key);if(G.scene.tilemap||G.scene.walkmask)ok(ARPIA_TILEMAP.plan(G.scene,{x:s.x,y:s.y},targets[0])?.length,'route '+r.key);
   ...
   ok(s.stage===at+1||(r.key==='monster'&&s.stage===at),'advance '+r.key);logs.push('PASS '+at+' '+r.key+' '+r.scene);
  }
  ok(s.moleStoryComplete26&&s.ugaugaAte26,'healed');ok(s.pets.some(p=>p.name==='밴드 두더지'),'new pet');ok(!s.inv.mt_gold26&&!s.inv.mt_lala26&&!s.inv.mt_wood26&&!s.inv.mt_ore26,'quest items consumed');out.textContent=logs.join('\n')+'\nALL PASS: episode 26, five monsters, healing, three gold and new pet.';
