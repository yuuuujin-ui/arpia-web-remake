document.getElementById('test-run').onclick=async()=>{
 ...
 try{
  const G=ARPIA,x=ARPIA_EXTRA;for(let i=0;!G.ready&&i<100;i++)await new Promise(r=>setTimeout(r,100));G.newGame();const s=G.state,c=ARPIA_MIDTERM.chapters.find(c=>c.number===23);
  ...
  ok(s.enemyInvitationLost&&!s.inv.mt_invitation23,'invitation lost');ok(s.enemyShaiyaTrust&&s.enemyFurFound&&s.enemyMirrorInjured,'investigation complete');out.textContent=logs.join('\n')+'\nALL PASS: episode 23, escape battle, two Hunger, Shaiya and evidence.';
