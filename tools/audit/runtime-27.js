document.getElementById('test-run').onclick=async()=>{
 ...
 try{
  const G=ARPIA,x=ARPIA_EXTRA;for(let i=0;!G.ready&&i<100;i++)await new Promise(r=>setTimeout(r,100));G.newGame();const s=G.state,c=ARPIA_MIDTERM.chapters.find(c=>c.number===27);
  ...
  ok(s.newspaperFirstIssue27&&s.inv.mt_paper27===1,'newspaper');ok(!s.inv.mt_wand27&&!s.inv.mt_tax27&&!s.inv.mt_gold27&&!s.inv.mt_letter27,'deliveries');out.textContent=logs.join('\n')+'\nALL PASS: episode 27, interviews, tax recovery, duels and newspaper.';
