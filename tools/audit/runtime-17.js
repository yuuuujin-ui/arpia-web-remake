document.getElementById('test-run').onclick=async()=>{
 ...
 try{
  const G=ARPIA,x=ARPIA_EXTRA;for(let i=0;!G.ready&&i<100;i++)await new Promise(r=>setTimeout(r,100));G.newGame();const s=G.state,c=ARPIA_MIDTERM.chapters.find(c=>c.number===17);
  ...
  ok(s.midtermBasicScore===100,'score');ok(ARPIA_SYS.inv.count(s,'mt_basic-certificate')===1,'certificate');out.textContent=logs.join('\n')+'\nALL PASS: episode 17, dungeon targets, item handovers, battle and exam.';
