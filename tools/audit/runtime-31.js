document.getElementById('test-run').onclick=async()=>{
 ...
 try{
  const G=ARPIA,x=ARPIA_EXTRA;for(let i=0;!G.ready&&i<100;i++)await new Promise(r=>setTimeout(r,100));G.newGame();const s=G.state,c=ARPIA_MIDTERM.chapters.find(c=>c.number===31);
  ...
  ok(s.kalamaLocated31&&s.kalamaFirstVisit31,'oasis');ok(!s.inv.mt_silkFruit31&&!s.inv.mt_navinium31&&!s.inv.mt_fatherCrystal31,'returned items');out.textContent=logs.join('\n')+'\nALL PASS: episode 31, family proof, teleport, Hunger and crystal return.';
