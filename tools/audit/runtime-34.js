document.getElementById('test-run').onclick=async()=>{
 ...
 try{
  const G=ARPIA,x=ARPIA_EXTRA;for(let i=0;!G.ready&&i<100;i++)await new Promise(r=>setTimeout(r,100));G.newGame();const s=G.state,c=ARPIA_MIDTERM.chapters.find(c=>c.number===34);
  ...
  ok(s.huntCount34===10,"ten ice golems");ok(s.fireMagicAwakened34&&s.paitierLessonComplete34,"fire lesson");ok(!s.inv.mt_lavaLadle34&&!s.inv.mt_fireKey34,"ladle returned and key used");out.textContent=logs.join('\n')+'\nALL PASS: episode 34, ten ice golems, ladle return, three key fragments and flame.';
