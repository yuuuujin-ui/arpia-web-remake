document.getElementById('test-run').onclick=async()=>{
 ...
 try{
  const G=ARPIA,x=ARPIA_EXTRA;for(let i=0;!G.ready&&i<100;i++)await new Promise(r=>setTimeout(r,100));G.newGame();const s=G.state,c=ARPIA_MIDTERM.chapters.find(c=>c.number===21);
  ...
  for(let tries=0;s.stage<c.end&&tries<70;tries++){const r=c.steps[s.stage-c.start],at=s.stage;G.close();G.travel(r.scene);const targets=G.scene.entities.filter(e=>e.id===r.npc);ok(targets.length===1,'target '+r.key);if(G.scene.tilemap||G.scene.walkmask)ok(ARPIA_TILEMAP.plan(G.scene,{x:s.x,y:s.y},targets[0])?.length,'route '+r.key);
   ...
   ok(s.stage===at+1||(r.key==='golems'&&s.stage===at),'advance '+r.key);logs.push('PASS '+at+' '+r.key+' '+r.scene);
  }
  ok(s.secretExamComplete,'exam complete');ok(s.huntCount21===10,'ten golems');ok(s.secretPouchRescued,'pouch rescued');ok(s.midtermBasicVoided&&s.midtermCombatVoided,'scores corrected');out.textContent=logs.join('\n')+'\nALL PASS: episode 21, ten golems, rescue, confession and final report.';
