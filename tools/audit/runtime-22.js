document.getElementById('test-run').onclick=async()=>{
 ...
 try{
  const G=ARPIA,x=ARPIA_EXTRA;for(let i=0;!G.ready&&i<100;i++)await new Promise(r=>setTimeout(r,100));G.newGame();const s=G.state,c=ARPIA_MIDTERM.chapters.find(c=>c.number===22);
  ...
  ok(s.enemyAlibis===15,'four alibis');ok(s.enemyGlugluSuspected&&s.enemyIceMeetingKnown,'investigation complete');out.textContent=logs.join('\n')+'\nALL PASS: episode 22, four alibis, Ryoma rescue and investigation.';
